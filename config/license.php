...
PEM),
    ...
    // This is the enforcement lag for a support suspension/expiry: shorter
    // means faster enforcement but more live calls to the license server
    // from routine Studio use (order status changes, courier assignment,
    // page loads — anywhere OperationalStanding::assertActive() runs), each
    // one adding that request's latency if the license server is slow.
    'verify_cache_minutes' => (int) env('LICENSE_VERIFY_CACHE_MINUTES', 60),
